...
抽出ルール：
1. 血液から測定される検査値のみを抽出する（事前定義の項目に限定しない）
2. 各項目を以下の形式で返す
3. 値が読み取れない項目はnullとする
4. 単位変換が必要な場合は変換して返す
5. 【重要】以下は血液検査値ではないため extractedBiomarkers に絶対に含めないこと：
   ・身体測定：身長、体重、標準体重、肥満度、BMI、体脂肪率、腹囲、内臓脂肪CT
   ・バイタル：収縮期血圧、拡張期血圧（血圧全般）
   ・眼科：視力（右・左）、眼圧、色覚
   ・耳鼻科：聴力（1000Hz・4000Hz）
   ・循環器：心電図所見、ABI、PWV
   ・呼吸器：胸部X線所見、肺活量、努力肺活量、1秒率
   ・超音波：腹部超音波所見、頸動脈超音波、心臓超音波、甲状腺超音波
   ・骨代謝：骨密度
   ・尿検査：尿比重、尿pH、尿蛋白、尿潜血、尿糖、ウロビリノーゲン、ケトン体、尿沈渣（赤血球・白血球・扁平上皮）
   ・便検査：便潜血、便虫卵
   ・問診・診察所見
   ・血液型（ABO・Rh）
   採血によって得られる数値データのみを抽出すること。
...
keyの命名規則（既存フィールドとの互換性のため以下に従う）：
  ...
  上記以外の項目 → 日本語名をローマ字またはスネークケース英語に変換して命名`
    ...
    console.log('AI response length:', responseText.length, 'stop_reason:', message.stop_reason);
    ...
    try {
      ...
      // If JSON is truncated (max_tokens hit), try to repair it
      ...
      // Remove trailing comma or incomplete entry
      ...
      try {
        ...
        console.error('JSON repair also failed:', repaired.substring(0, 500));
        return res.status(500).json({ success: false, error: 'レスポンスのJSON解析に失敗しました。再度お試しください。' });
      }
    }
    ...
    return res.status(500).json({
      success: false,
      error: '解析中にエラーが発生しました: ' + errorMessage.substring(0, 200),
    });
  }
}
